 ...
 * @return boolean
 */
function topiccal_forum_check($forum_id) 
{
	global $db;
 ...
 * @return void
 */
function topiccal_submit_event($mode, $forum_id, $topic_id, $post_id)
{
	global $db, $user;
	...
	// setup defaults
	$cal_isodate = topiccal_date2iso(request_var('cal_date', $user->lang['NO_DATE']));
	$cal_repeat = 1;
	...
	if ($cal_isodate && $cal_interval_date && ($cal_date_end != $user->lang['NO_DATE'] || $cal_repeat_always))
	{
		...
		{
			$cal_isodate_end = topiccal_date2iso($cal_date_end);
	...
	if ($mode == 'post' && $cal_isodate && topiccal_forum_check($forum_id))
	{
	...
	else if ($mode == 'edit' && topiccal_forum_check($forum_id))
	{
		// check if not allowed to edit the calendar event since this is not the first post
		if (!topiccal_first_post($topic_id, $post_id))
		{
			return;
		}
...
function topiccal_move_event($new_forum_id, $topic_id, $leave_shadow = false)
{
	global $db;
	
	// if we are not leaving a shadow and the new forum doesn't do events,
	// then delete to event and return
	if (!$leave_shadow && !topiccal_forum_check($new_forum_id))
	{
		if (!$leave_shadow)
		{
//			topiccal_delete_event($topic_id, null, false);
			return;
		}
	} else
	{
 ...
 */
function topiccal_show_event($topic_id, $post_id) 
{
	global $db, $user;
	...
	if ($row)
	{
		...
		$cal_repeat = $row['cal_repeat'];
		$event['message'] = '<i>' . topiccal_translate_date($cal_date_f) . '</i>';
		...
		if ($row['cal_repeat'] != 1)
		{
			...
			if ($row['cal_repeat'] > 1)
			{
				...
				$event['message'] .= ' - <i>' . topiccal_translate_date($cal_date_end) . '</i>';
			}
 ...
 * @return void
 */
function topiccal_generate_entry($mode, $forum_id, $topic_id, $post_id, &$template) 
{
	global $db, $user;

	// if this is a reply/quote or not an event forum or if we are editing and it is not the first post, just return
	if ($mode == 'reply' || $mode == 'quote' || !topiccal_forum_check($forum_id) || ($mode == 'edit' && !topiccal_first_post($topic_id, $post_id)))
	{
		return;
	}
	...
	$interval_units = array(
		'DAY'   => topiccal_pluralize($user->lang['INTERVAL']['DAY'], $user->lang['INTERVAL']['DAYS']),
		'WEEK'  => topiccal_pluralize($user->lang['INTERVAL']['WEEK'], $user->lang['INTERVAL']['WEEKS']),
		'MONTH' => topiccal_pluralize($user->lang['INTERVAL']['MONTH'], $user->lang['INTERVAL']['MONTHS']),
		'YEAR'  => topiccal_pluralize($user->lang['INTERVAL']['YEAR'], $user->lang['INTERVAL']['YEARS']),
	);
	...
	topiccal_base_calendar();
}

function topiccal_base_calendar()
{
 ...
 */
function topiccal_date2iso($in_stringDate) 
{
	global $user;
 ...
 */
function topiccal_first_post($topic_id, $post_id)
{
	global $db;
...
function topiccal_translate_date($in_date)
{
	global $user, $config;
 ...
 */
function topiccal_pluralize($in_singular, $in_plural)
{
